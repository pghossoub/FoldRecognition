...
while(my $line=<F>)
{
	...
	else 
	{
		...
		{
			my $aa=$tab_line[$i];
			#print "INDEX: $index HASH POSITION: $hash_position_filtered{$index} NUMBER_SEQ:$number_seq\n";
			#my $fraction_gap=$hash_position_filtered{$index}/$number_seq;
			#if ($fraction_gap < 0.05)
			#{
